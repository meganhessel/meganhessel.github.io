...
Between February 11 and 20, 2021, Winter Storm Uri swept across Texas, bringing record-breaking cold tempertures and heavy precipitation (NOAA, 2023). The storm produced the longest stretch of freezing temperatures in the state's history, triggering widespread power outages, burst pipes, dangerous road conditions, and numerous accidents (NOAA, 2023 and Watson P, 2021). At some point during the storm, millions of civilians (more than 2 out of 3 people) were left with no electricity or heat during the subfreezing conditions (Watson P, 2021).
...
After creating a complete night light image of Feb 7 and 16, we need to make sure the extents of both days are the same. `Extents` refer to the geospatial range encompassed within the data.
...
**Blackouts** are when the power difference (between pre and post night light data) is greater than 200 nW cm^-2^sr^-1^. We can find this by using mathematics operations between tiles and logicals.
...
**Mask** means to convert and/or filter for specific data. In this case, we are masking for *only* areas that experienced a blackout. Anywhere that did not experience a blackout gets an NA value.
...
**GOAL:** Map Houston's night light pre and post the Winter Storm.
...
[Plot]{.underline} the light intensity between Feb 7 and Feb 16 to see the considerable amount of blackouts from the storm.
...
Now, we need to find the homes that experienced a blackout! The function `st_filter` (with .predicate = st_intersects) will find all the houses that are within the blackout cropped mask.
...
Filtering for homes that did vs did not experience a blackout and merging the home dataset to the Texas census track data.

**GOALS:**

1.  Make a datasets with homes that *DID* experience a blackout and their median household income = `income_bo_homes`

2.  Make a datasets with homes that *did NOT* experience a blackout and their median household income = `income_NO_bo_homes`

\(1\) Find the intersection of homes that **experienced blackouts** and the census track data with `st_filter` (and `.predicate = st_intersects`).
...
\(2\) Using the function `st_intersections` (with 'sparse = FALSE'), find the locations where homes and census tracks do (TRUE) and do not (FALSE) intersect. Then, we filter for the locations with **no blackouts** (the areas that do not intersect - the FALSEs).
...
Mapping the median household income distributions if homes did vs did not experience a blackout.
...
Plot income Ccasses in terms of experiencing power outages or not.
...
[Find *Precentages* of Income Classes that Experienced a Blackout]{.underline}

To find the percentages of each income class that experiences a blackout, we will find the population within each income bracket, filter for the population that lost power in each class, then calculate the precentage (count that experienced a blackout/total count of income bracket population).
